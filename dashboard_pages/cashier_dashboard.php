    ...
    <nav>
        ...
        <h2>Welcome, <?php session_start();
                        ...
                        if (!isset($_SESSION['role']) || $_SESSION['role'] !== 'cashier') {
                            // Redirect to the login page if not logged in as a cashier
                            header("Location: ../auth/cashier_login.php");
                            exit();
                        }
                        // Now you can safely access the session data without undefined index warnings
                        // $userId = $_SESSION['user_id'];
                        $email = $_SESSION['email'];
                        $role = $_SESSION['cashier'];
                        // $email = $_SESSION['email'];
                        echo $email, '(' . $role . ')'; ?> </h2>
    </nav>
